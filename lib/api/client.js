...
class ApiClient {
  ...
  // expose HTTP status to callers so they can handle specific statuses (e.g., 404) gracefully
  ...
  async unclaimItem(itemId) {
    try {
      return await this.request(`/api/items/${itemId}/unclaim`, {
        method: 'DELETE',
      });
    } catch (err) {
      // If the item is already gone/unclaimed, treat 404 as idempotent success
      if (err && err.status === 404) {
        return { success: true, alreadyUnclaimed: true };
      }
      throw err;
    }
  }
