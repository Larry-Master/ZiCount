...
export const useReceiptUpload = () => {
  ...
  const uploadToGCS = useCallback(async (file) => {
    try {
      ...
      const { uploadUrl, gcsUrl, readUrl } = await response.json();
      ...
      // return both the GCS URL (for Document AI) and a signed read URL for preview
      return { gcsUrl, readUrl };
    } catch (err) {
  ...
  const analyzeReceipt = useCallback(async (receiptData) => {
    ...
    try {
      // Upload to GCS first (we send the original file as-is for Document AI)
      // Note: do NOT compress the file before analysis — Document AI should get the original image
      const { gcsUrl, readUrl } = await uploadToGCS(selectedImage);
      ...
      // Handle image optimization for storage and preview.
      // Important: avoid referencing the uploaded GCS object for previews (don't use readUrl/publicUrl),
      // and do NOT rely on server-returned imageBase64 (could be large). Prefer the client-side preview
      // we already created when the user selected the file. If that is missing, create a preview now.
      let imageUrlToStore = imagePreview;
      if (!imageUrlToStore) {
        try {
          imageUrlToStore = await createPreviewFromFile(selectedImage);
        } catch (previewErr) {
          console.warn('Failed to create preview after analysis:', previewErr);
  ...
  }, [selectedImage, uploadToGCS, imagePreview]);
