...
export const useReceiptUpload = () => {
  ...
  const analyzeReceipt = useCallback(async (receiptData) => {
    ...
    try {
  // Upload to GCS first (also receive a signed read URL for preview)
  ...
  // Prefer returned imageBase64 from server if present; otherwise prefer signed read URL so
  // the client can fetch the uploaded image for preview without embedding large base64 in the API response.
  let imageUrlToStore = readUrl || publicUrl;
      if (data.imageBase64) {
        try {
