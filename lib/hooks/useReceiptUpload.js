...
export const useReceiptUpload = () => {
  ...
  const analyzeReceipt = useCallback(async (receiptData) => {
    ...
    try {
      ...
      // If the analysis endpoint returned an embedded image, prefer that.
      // Otherwise fallback to the client-side preview (so saved receipts still
      // have a visible image even if the server deleted the GCS object).
      let imageUrlToStore = imagePreview || publicUrl;
      if (data.imageBase64) {
        try {
