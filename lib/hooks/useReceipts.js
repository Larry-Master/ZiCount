...
export const useClaims = () => {
  ...
  const claimMutation = useMutation({
    ...
    onSettled: (data, error, { receiptId, itemId, userId }) => {
      queryClient.invalidateQueries({ queryKey: ['receipt', receiptId] });
      queryClient.invalidateQueries({ queryKey: ['receipts'] });
      if (userId) {
  ...
  const unclaimMutation = useMutation({
    ...
    onSettled: (data, error, { receiptId, itemId, userId }, context) => {
      const targetUserId = userId || context?.claimedByUserId;
      console.log('[Unclaim] Settled - invalidating caches for userId:', targetUserId);
      
      // Invalidate to trigger background refetch (not forced)
      queryClient.invalidateQueries({ queryKey: ['receipt', receiptId] });
      queryClient.invalidateQueries({ queryKey: ['receipts'] });
      
      // userClaims already removed in onMutate, just ensure it's gone
      if (targetUserId) {
