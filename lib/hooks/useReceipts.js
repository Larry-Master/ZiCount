...
export const useClaims = () => {
  ...
  const claimMutation = useMutation({
    ...
    onSuccess: async (data, { receiptId, itemId, userId }) => {
      console.log('[Claim] Success - refetching data');
      
      // Wait for the refetch to complete before clearing optimistic state
      await Promise.all([
        queryClient.refetchQueries({ queryKey: ['receipt', receiptId] }),
        queryClient.refetchQueries({ queryKey: ['receipts'] })
      ]);
      
      // Now clear optimistic state after fresh data is loaded
      optimisticRef.current.delete(itemId);
    ...
    onSettled: (data, error, { itemId }) => {
      // Cleanup: ensure optimistic state is removed even if onSuccess fails
      if (error) return; // Already handled in onError
      
      // Double-check optimistic state is cleared
      setTimeout(() => {
        optimisticRef.current.delete(itemId);
      }, 100);
    }
  ...
  const unclaimMutation = useMutation({
    ...
    onSuccess: async (data, { receiptId, itemId, userId }, context) => {
      ...
      console.log('[Unclaim] Success - refetching data for userId:', targetUserId);
