...
export const useClaims = () => {
  const queryClient = useQueryClient();
  // track optimistic claims so components can read immediate state
  const optimisticClaimsRef = useRef(new Map());
  ...
  const claimMutation = useMutation({
    ...
    onMutate: async ({ receiptId, itemId, userId }) => {
      ...
      // store optimistic claim for components that read the map
      optimisticClaimsRef.current.set(itemId, { claimedBy: userId, claimedAt, pendingClaim: true });

      // Optimistically update single receipt
      if (prevReceipt) {
        const newReceipt = {
          ...prevReceipt,
          items: (prevReceipt.items || []).map(it => it.id === itemId ? { ...it, claimedBy: userId, claimedAt, pendingClaim: true } : it)
        };
    ...
    onError: (err, variables, context) => {
      // rollback
      const { receiptId, itemId } = variables || {};
      optimisticClaimsRef.current.delete(itemId);
      if (context?.prevReceipt && receiptId) {
        queryClient.setQueryData({ queryKey: ['receipt', receiptId] }, context.prevReceipt);
      }
      if (context?.prevReceipts) {
        queryClient.setQueryData({ queryKey: ['receipts'] }, context.prevReceipts);
    ...
    onSettled: (data, error, variables) => {
      // remove optimistic marker for this item and refetch to ensure consistency
      if (variables?.itemId) optimisticClaimsRef.current.delete(variables.itemId);
  ...
  const unclaimMutation = useMutation({
    ...
    onMutate: async ({ receiptId, itemId }) => {
      ...
      // mark optimistic unclaim in the map
      optimisticClaimsRef.current.set(itemId, { claimedBy: null, claimedAt: null, pendingUnclaim: true });
    ...
    onError: (err, variables, context) => {
      // rollback optimistic map and cache
      const { itemId, receiptId } = variables || {};
      if (itemId) optimisticClaimsRef.current.delete(itemId);
      if (context?.prevReceipt && receiptId) {
        queryClient.setQueryData({ queryKey: ['receipt', receiptId] }, context.prevReceipt);
      }
      if (context?.prevReceipts) {
  ...
  return {
    claims: [],
    // expose the optimistic claims map so components can read optimistic state
    optimisticClaims: optimisticClaimsRef.current,
