 ...
 */
export const useReceipt = (receiptId, { pollIntervalMs = 15000 } = {}) => {
  // visibility-aware polling: only poll when tab is visible to save battery/data
  const isClient = typeof window !== 'undefined';
  const query = useQuery({
    queryKey: ['receipt', receiptId],
    queryFn: () => apiClient.getReceipt(receiptId),
    enabled: !!receiptId,
...
export const useUserClaims = (userId, { pollIntervalMs = 20000 } = {}) => {
  const isClient = typeof window !== 'undefined';
  const query = useQuery({
    queryKey: ['userClaims', userId],
    queryFn: () => apiClient.getUserClaims(userId),
    enabled: !!userId,
    refetchInterval: isClient ? () => (document.visibilityState === 'visible' ? pollIntervalMs : false) : false,
    refetchOnWindowFocus: true,
  });
