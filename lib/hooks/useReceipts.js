...
export const useReceiptMutations = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSettled: (data, error, receiptId) => {
      ...
      // Invalidate any cached user claims so MyClaims refreshes after a receipt deletion
...
export const useClaims = () => {
  const queryClient = useQueryClient();
  // Stable map to hold optimistic claim/unclaim states for items
  ...
  const claimMutation = useMutation({
    ...
    onMutate: async ({ receiptId, itemId, userId }) => {
      ...
      if (prevReceipt) {
        const newReceipt = {
          ...prevReceipt,
          items: (prevReceipt.items || []).map(it => it.id === itemId ? { ...it, claimedBy: userId, claimedAt: new Date().toISOString(), pending: true } : it)
        };
      ...
      if (prevReceipts) {
        const newList = (prevReceipts || []).map(r => {
          ...
          return {
            ...r,
            items: (r.items || []).map(it => it.id === itemId ? { ...it, claimedBy: userId, pending: true } : it)
          };
        });
        queryClient.setQueryData({ queryKey: ['receipts'] }, newList);
      }

      // store optimistic snapshot for components to read
      optimisticRef.current.set(itemId, { id: itemId, claimedBy: userId, claimedAt: new Date().toISOString(), pending: true });
    ...
    onError: (err, variables, context) => {
      ...
      // cleanup optimistic map on error
      if (variables?.itemId) optimisticRef.current.delete(variables.itemId);
    },
    onSettled: (data, error, variables) => {
      queryClient.invalidateQueries({ queryKey: ['receipt', variables.receiptId] });
      queryClient.invalidateQueries({ queryKey: ['receipts'] });
      // Also refresh the claiming user's claims list so MyClaims shows the new claim quickly
      if (variables?.userId) {
        queryClient.invalidateQueries({ queryKey: ['userClaims', variables.userId] });
      }
      // remove optimistic snapshot (server result or rollback will populate real data)
      if (variables?.itemId) optimisticRef.current.delete(variables.itemId);
    }
  });
  ...
  const unclaimMutation = useMutation({
    ...
    onMutate: async ({ receiptId, itemId }) => {
      ...
      if (prevReceipt) {
        const newReceipt = {
          ...prevReceipt,
          items: (prevReceipt.items || []).map(it => it.id === itemId ? { ...it, claimedBy: null, claimedAt: null, pending: true } : it)
        };
        queryClient.setQueryData({ queryKey: ['receipt', receiptId] }, newReceipt);
      }
      ...
      if (prevReceipts) {
        const newList = (prevReceipts || []).map(r => {
          ...
          return {
            ...r,
            items: (r.items || []).map(it => it.id === itemId ? { ...it, claimedBy: null, pending: true } : it)
          };
        });
        queryClient.setQueryData({ queryKey: ['receipts'] }, newList);
      }

      // store optimistic unclaim state
      optimisticRef.current.set(itemId, { id: itemId, claimedBy: null, claimedAt: null, pending: true });
      return { prevReceipt, prevReceipts };
    },
    onError: (err, variables, context) => {
      if (context?.prevReceipt) {
