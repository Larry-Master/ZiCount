...
export const useClaims = () => {
  ...
  const claimMutation = useMutation({
    ...
    onSuccess: async (data, { receiptId, itemId, userId }) => {
      console.log('[Claim] Success - invalidating queries');
      
      // Use invalidateQueries instead of refetchQueries to mark data as stale
      // This allows the optimistic state to persist until user navigates or data is needed
      queryClient.invalidateQueries({ queryKey: ['receipt', receiptId] });
      queryClient.invalidateQueries({ queryKey: ['receipts'] });
      
      // Clear optimistic state after a delay to ensure smooth UX
      setTimeout(() => {
        optimisticRef.current.delete(itemId);
      }, 500);
    ...
    onSettled: (data, error, { itemId }) => {
      // Cleanup: ensure optimistic state is removed eventually
      if (error) return; // Already handled in onError
      
      // Final cleanup after UI has updated
      setTimeout(() => {
        optimisticRef.current.delete(itemId);
      }, 1000);
    }
  ...
  const unclaimMutation = useMutation({
    ...
    onSuccess: async (data, { receiptId, itemId, userId }, context) => {
      ...
      console.log('[Unclaim] Success - invalidating queries for userId:', targetUserId);
    ...
    onSettled: (data, error, { receiptId, itemId }) => {
      // Cleanup: ensure optimistic state is removed eventually
      if (error) return; // Already handled in onError
