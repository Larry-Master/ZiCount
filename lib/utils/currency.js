 ...
 * - Receipt API objects with price.value structure
 * 
 * @param {number|string|Object} priceInput - Price in various formats
...
export const parsePrice = (priceInput) => {
  ...
  // Handle object prices (common in receipt items)
  if (typeof priceInput === 'object' && priceInput.value !== undefined) {
    return parsePrice(priceInput.value); // Recursive call for nested objects
  }
  
  // Clean and standardize price string format
  const cleanPrice = priceInput.toString()
